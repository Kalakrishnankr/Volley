		...
		JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(ApiService.REQUEST_METHOD_POST, ApiService.EVENT_REGISTER, object,
		    new Response.Listener<JSONObject>() {
			    ...
			    public void onResponse(JSONObject response) {
				    if (response != null) {
					    Toast.makeText(getActivity(), R.string.you_have_been_successfully_registered_for_the_event, Toast.LENGTH_SHORT).show();
					    ...
					    event.setEventStatus("Registered");
					    event.setRegType("Organizer");
					    EventViewModel vm = ViewModelProviders.of(getActivity()).get(EventViewModel.class);
					    vm.setChangedEvent(getArguments().getInt("selectedPosition"), event);
				    }
