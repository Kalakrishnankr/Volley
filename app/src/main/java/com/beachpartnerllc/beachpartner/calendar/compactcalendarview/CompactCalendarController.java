package com.beachpartnerllc.beachpartner.calendar.compactcalendarview;
...
import static com.beachpartnerllc.beachpartner.calendar.compactcalendarview.CompactCalendarView.CompactCalendarViewListener;
import static com.beachpartnerllc.beachpartner.calendar.compactcalendarview.CompactCalendarView.FILL_LARGE_INDICATOR;
import static com.beachpartnerllc.beachpartner.calendar.compactcalendarview.CompactCalendarView.NO_FILL_LARGE_INDICATOR;
import static com.beachpartnerllc.beachpartner.calendar.compactcalendarview.CompactCalendarView.SMALL_INDICATOR;
...
class CompactCalendarController {
            ...
            for (int i = 0; i < uniqEvents.size(); i++) {
                ...
                if (shouldDrawIndicatorsBelowSelectedDays || (!shouldDrawIndicatorsBelowSelectedDays && !isSameDayAsCurrentDay && !isCurrentSelectedDay) || animationStatus == EXPOSE_CALENDAR_ANIMATION) {
                    if (eventIndicatorStyle == FILL_LARGE_INDICATOR || eventIndicatorStyle == NO_FILL_LARGE_INDICATOR) {
                        ...
                        drawTextEvents(canvas,xPosition,yPosition,eventsList);
    ...
    private void drawSingleEvent(int eventSize,Canvas canvas, float xPosition, float yPosition, List<Event> eventsList) {
        float indicatorOffset = -bigCircleIndicatorRadius-bigCircleIndicatorRadius/4;
...
//        dayPaint.setTextSize(40);
    ...
    //To draw circle around the number of events
    private void drawEventCircleIndicator(int indicatorStyle, Canvas canvas, float x, float y, int color) {
        drawDayCircleIndicator(indicatorStyle, canvas, x, y, color, (float) 0.4);
    }
